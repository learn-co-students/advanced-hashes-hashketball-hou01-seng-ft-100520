...
def team_colors(name)
  team = game_hash.find {|key, hash| hash[:team_name] == name}
...
def player_numbers(team_name)
  nums = []
  team = game_hash.find {|key, hash| hash[:team_name] == team_name}
...
def big_shoe_rebounds
  max = -1
  players = game_hash[:away][:players] + game_hash[:home][:players]
  players.each do |player|
    if player[:shoe] > max
      max = player[:shoe]
    end
  end
  player = game_hash[:away][:players].find {|hash| hash[:shoe] == max} || game_hash[:home][:players].find {|hash| hash[:shoe] == max}
